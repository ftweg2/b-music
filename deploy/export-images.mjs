...
const services=process.argv.slice(3).length?process.argv.slice(3):["app","kernel"];
if(services.some(service=>!["app","kernel"].includes(service)))throw new Error("Unknown service");
const directory=path.join(import.meta.dirname,"bundles");fs.mkdirSync(directory,{recursive:true});
const file=path.join(directory,(services.length===2?"images":services[0])+"-"+tag+".tar.gz");
const child=spawn("docker",["save",...services.map(service=>"bmusic-"+service+":"+tag)],{stdio:["ignore","pipe","inherit"]});
